...
for (sensor in types){
  ...
  assign(nam,do.call(rbind.data.frame,parallel::mclapply(
    dir_all_filenames,readr::read_delim,delim =";", show_col_types = FALSE, na=c("NA","NaN"))))
}
...
is_in_muc <- function(lon, lat){
  sp::point.in.polygon(point.x = lon, point.y = lat, 
                       pol.x = munich_pol[[1]][1][[1]][[1]][,1], pol.y = munich_pol[[1]][1][[1]][[1]][,2])
}
...
write.table(unlist(sensors_name_id_munich), 
            paste(path_workfiles, "workfiles", "sensors_munich.txt", sep = "/"),
